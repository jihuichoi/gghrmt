...
var (
	...
	// timeFormat is custom Time Foramt
...
// custromTimeEncoder encode Time to our custom format
...
// timeFormat - custom time format for logger of empty sting to use default
func Init(lvl int, timeFormat string) error {
	...
	onceInit.Do(func() {
		...
		// output should also go to standard error
		// It is useful for Kubernetes deployment.
		// Kunernetes interprets os.Stdout log items as INFO and os.Stderr log items
		// as ERROR by default
		...
		consoleErorrs := zapcore.Lock(os.Stderr)
		...
		// zapcore
		core := zapcore.NewTee(
			zapcore.NewCore(consoleEncoder, consoleErorrs, highPriority),
